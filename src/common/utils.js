...
//      user = JSON.parse(user);
  ...
  checkAuthorities: function (_router, permission) {
    if (permission !== 'Login' && permission !== 'Logout') { // 校验权限
      ...
      if (!authorities) {
        ...
        // 跳转到统一授权失败页面
        window.location = 'http://ucenter.xconline.edu/#/denied?returnUrl=' + Base64.encode(window.location)
/*        router.push({
          path: '/login'
        }) */
      }
      // console.log(authorities)
      var ret1 = authorities.find((value, _index, _arr) => {
